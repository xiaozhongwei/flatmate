  ...
  listingRouter.get('/:id', function(req, res) {
    res.send({
      'listing': {
        ...
        "status": 1,
        "links":{
          "similarListings":"/api/listings/" + req.params.id + "/similar"
        }
      }
    });
  });

  listingRouter.get('/:id/similar', function(req, res) {
    res.send({
      'listings': [{
        ...
        "features": ["Balcony","PrivateBath","DeskOrWardrobe"],
        "photos": [
          {id: Math.ceil(Math.random() * 10000), index: 1, imageUrl: "images/post-5.jpg"},
          {id: Math.ceil(Math.random() * 10000), index: 0, imageUrl: "images/post-5.jpg"}
        ],
        "house": {
          "id": "3",
          "bathrooms": 3,
